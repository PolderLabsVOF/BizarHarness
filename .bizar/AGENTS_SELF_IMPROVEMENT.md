...
1. **Read `.obsidian/INDEX.md` before any project-specific work.** The architecture note is updated by Mimir at start of major sessions.
2. **For multi-stream parallel work: disjoint file scopes + PARALLEL EXECUTION CONTEXT block per agent.** Define `Owns` (read+write) and `Siblings READ-ONLY` upfront.
3. **Resolve pre-existing merge conflict markers BEFORE parallel work.** Grep for `<<<<<<<` across the tree; clear with Thor cleanup agent before dispatching streams.
4. **`npx tsc --noEmit` is the cheapest correctness check.** Run after every meaningful edit pass.
5. **Run `node --test` across all relevant test files before declaring done.** Pre-existing failures are noted separately; new failures must be fixed.
6. **`routes/*.mjs` are append-only-friendly.** New endpoints → new files in `routes/`, mounted in `api.mjs`. Avoid editing existing route files when parallel work is in flight.
7. **Hand-rolled SVG charts avoid dependencies.** When adding a chart, build it inline; don't add a chart library.
8. **Bundled skills live in `bizar-dash/skills/`.** Shipped with the npm package so they're available even on a fresh install.
9. **WebSocket bus is the central nervous system.** New entity types → new WS message types in `WsMessage` union.
10. **No plaintext API keys anywhere.** Always env vars; managed via `~/.config/bizar/env.json` (mode 0600).
...
18. **Browser extension MV3 `chrome.runtime.sendMessage` must be wrapped defensively.** Chrome can terminate the service worker at any time, causing synchronous throws on disconnected ports. Every extension message needs a try/catch and a fallback path (e.g. direct `fetch()` to the dashboard API).
## Log

### 2026-07-06 — v5.5.1 — Steering followup + UI overhaul + server log fixes + browser extension cleanup
...
### 2026-07-06 — v5.5.0 — Background agents dashboard + memory system + installer overhaul

- **Task**: Ship three parallel implementation streams (background agents full dashboard integration, memory system full integration, installer end-to-end overhaul) plus an 8-issue bug-fix sweep and strategic docs rewrite in one release.
...
- **Agents used**: none — `task` and `bizar_spawn_background` still broken in this Odin environment (per earlier diagnosis). Work proceeded directly with `edit`/`write`/`bash`.
- **Sibling awareness**: No siblings active this turn — direct-edit mode.
- **Verification gates**:
...
- `routes/*.mjs` files that get mounted by `api.mjs` are easy to extend additively — adding a new file is safer than editing existing ones when multiple agents are writing.
- Hand-rolled SVG charts avoid heavy dependencies and match the dashboard's design system. Bundle stays small (build output: 317KB main, 475KB mobile).
- Pre-existing merge conflict markers (from prior sessions) leak into TS as `TS1185: Merge conflict marker encountered`. Always grep for `<<<<<<<` before declaring a file clean.
- Tests split across `node --test`, `bun test`, and `vitest` — total 786 tests across 3 runners. Pre-existing failures (9 in `submit-feedback.test.mjs`, 1 in `mod-upgrade.node.test.mjs`) are out of scope for this release.
- `tsc --noEmit` is the cheapest correctness check; running it before declaring done saved time.
**Pattern to follow next time:**
- Always read `.obsidian/INDEX.md` first for project context if it exists.
- For multi-stream parallel work: one prompt per stream with a `PARALLEL EXECUTION CONTEXT` block listing siblings + file scopes.
- Resolve pre-existing merge conflicts BEFORE parallel work begins.
- After parallel streams: a single cleanup agent runs the full test gate + build.

### 2026-07-05 — v4.5.1 Headroom + Memory tab

**Context:** The user asked to make Headroom the default compression layer for token efficiency and to add a full Memory tab to the dashboard. Three parallel streams delivered this.

**Files changed (v4.5.1):** 33 files: headroom integration module, Memory view + 5 panel components, settings + Sidebar + Topbar wiring, memory endpoints extended, `bizar headroom` CLI subcommand, skills updated.

**Agents used:** thor-headroom (Headroom integration), tyr-memory (Memory tab), heimdall-tasks (added tasks to Bizar tasks store).
...
- Auto-wrap/auto-start hooks should be in a dedicated startup function with try/catch — startup must never fail due to optional tools like Headroom.
- Test scripts must be kept in sync with new test files. Consider a glob-based test discovery script for `bizar-dash/tests/`.

### 2026-07-05 — v4.5.2 bug-fix sweep (16 fixes)

**Context:** Six parallel research streams did a comprehensive scan of the entire BizarHarness codebase and catalogued every issue. Three fix streams resolved 16 high-confidence bugs. User explicitly said auth is NOT needed — Tailscale handles it.

**Files changed:** ~20 files across CLI (`cli/bin.mjs`, `cli/provision.mjs`, `cli/doctor.mjs`, `cli/memory.mjs`, `cli/artifact.mjs`), installer (`install.sh`, `install.ps1`, `scripts/check-deps.mjs`), server (`bizar-dash/src/server/{providers-store,server,routes/chat,memory-lightrag}.mjs`), frontend (`App.tsx`, `Topbar.tsx`, `Toast.tsx`, 5 views), build (`vite.config.ts`, `.npmignore`), and styles.

**Agents used:** thor-cli-bugs (10 fixes), tyr-server-bugs (4 fixes), thor-frontend-bugs (6 fixes + 2 perf).

**Lessons learned:**
- **User confirmation matters.** The user explicitly said auth is handled by Tailscale. Initial recon flagged "v1 routes unauthenticated" as a critical issue, but it was intentional. Always confirm the security model before adding auth.
- **Bug-fix sweeps benefit from file-scoped parallel streams.** Each agent owned a disjoint set of files. Zero collisions.
- **Surgical edits beat rewrites.** Every fix was a single concern in 1-3 edits. No file rewrites.
- **`node --check` on every modified .mjs file** is a 50ms sanity check that catches syntax errors before they propagate.
- **Comprehensive research before fixing** uncovered things I would never have found otherwise (e.g., the install.ps1 syntax errors that have been there since v3.x).

**Pattern to follow next time:**
- For any large codebase sweep: parallel research streams (6 worked great) → synthesis to ROADMAP.md → parallel fix streams → test gate → release.
- Add `--json` and `--debug` flags as part of every CLI's foundation, not as an afterthought.
- `content-visibility: auto` is a free win for any list with > 50 items.
- `React.memo` on view components prevents the entire dashboard from re-rendering on every 5s snapshot poll.

### Active Rules updated:
- Always confirm the security model (Tailscale? nginx? Caddy?) before adding auth to anything.
- `node --check` on every modified .mjs file before declaring done.
- For comprehensive sweeps: research first (6 streams), synthesize second (1 stream), fix third (3 streams), test last.

### 2026-07-05 — v4.7.0 (v4.6 + v4.7 combined: Quality & Stability + Performance & Polish)

**Context:** User asked to fix all mentioned bugs and implement v4.6 + v4.7 in one go. Four parallel streams delivered the full release.

**Files changed:** 43 files. CLI refactor split two 2000+-line monoliths. Server added structured logger + metrics endpoint + cache-control. Web added virtual scrolling + i18n + a11y + 75 vitest tests.

**Agents used:** thor-cli (CLI refactor), tyr-server (logger + metrics + cache-control), thor-frontend (VirtualList + i18n + a11y), heimdall-tests (vitest + RTL setup).

**Lessons learned:**
- **Major CLI refactors need regression testing upfront.** The split of bin.mjs/artifact.mjs broke existing tests because they imported from the old paths. The cli-bugfixes.test.mjs import path had to be updated to `cli/commands/install.mjs`. Same for memory-cli.test.mjs. Pre-flight grep of test imports would have caught this.
- **`--help` flag handling is global vs per-subcommand.** Refactoring the help flow broke `bizar --help` AND `bizar memory setup --help`. The fix: detect "only one global flag" vs "command + --help" before dispatch.
- **A 4-stream parallel refactor of a 5000+ line codebase works.** All 4 streams finished without file collisions. The PARALLEL EXECUTION CONTEXT block at the top of each prompt was effective.
- **vitest + @testing-library/react setup takes ~30 minutes.** The `bizar-dash/vitest.config.ts` config + `setup.ts` is the minimum viable scaffold. jsdom env + RTL globals is enough for component tests.
- **Hand-rolled VirtualList beats react-window for our needs.** 4 views already migrated, no new dependency.

**Pattern to follow next time:**
- Before splitting a CLI monolith, grep for `from.*<old-path>` in test files to find tests that need import path updates.
- Always check both `bizar --help` AND `bizar <cmd> --help` after help-text refactors.
- For structured logging migrations, update existing tests to match new log format (`console.warn` → `logWarn`).
- **v5.0 Web Clipper + Screenshot OCR added.** Browser extension (Manifest V3) under `browser-extensions/bizar-clipper/`, bookmarklet under `bookmarklet/bizar-clipper.js`. Server routes `clipboard.mjs` + `ocr.mjs` mounted in api.mjs, CLI commands `clip` + `ocr` registered in bin.mjs. Frontend components `ScreenshotCapture`, `ScreenshotOCR`, `FromScreenshotPanel`, `VaultFromClipboardPanel` follow existing Card + Button + toast patterns. `tesseract.js` added as dependency for OCR. All 11 tests pass (6 node, 5 vitest), tsc --noEmit clean.
- **OCR route needs try/catch around `getOcr()` import.** The lazy import of `ocr.mjs` (which imports `tesseract.js`) throws MODULE_NOT_FOUND if tesseract.js is missing. Wrap in try/catch returning 503 to avoid 500 crash on empty body requests.
- **node:test OCR tests require conditional skip.** When tesseract.js is not installed, the OCR helper tests must be skipped (not fail). Use `{ skip: !moduleVar }` pattern in node:test.
- **Node.js test server must use port 0, not hardcoded port.** Use `server.listen(0)` and read `server.address().port` for dynamic port allocation to avoid port conflicts.

### 2026-07-05 — v5.0.1 bug-fix polish (10 fixes)

**Context:** User reported 4 issues in issues.md + 4 follow-ups. Plus compaction needed building from scratch + layout fix.

**Files changed:** ~30 files across dashboard, plugin, configs, docs.

**Agents used:** thor-settings-redesign, tyr-doctor, thor-autosave, heimdall-chat-fix (later tyr-chat-fix), thor-vault-default, thor-coming-soon, thor-minimax-models, thor-compaction, thor-layout, tyr-roadmap.

**Lessons learned:**
- **Empty catch blocks + missing error context = silent failures.** The chat network error was actually a 502 from the dashboard, not a client-side fetch failure. Structured logging at every error site saves hours of debugging.
- **Lay out top-level page padding at the framework level, not per-view.** The layout fix should have been part of the v4.5 design system.
- **Build from scratch when needed.** Compaction didn't exist — instead of routing back to Odin, building it inline in one stream was faster.

**Pattern to follow next time:**
- Always check if a feature exists before assuming it does (Thor did good work)
- For UI polish items, a single CSS sweep often catches multiple pages at once

### 2026-07-06: All 8 issues.md fixes + strategic doc rewrite + 3 high-priority parallel implementation streams

**Context:** Today's session implemented all 8 fixes from issues.md (Settings sidebar styling/buttons, Marketplace distinct page, Schedules tab-btn CSS, Memory default path, auto-start LightRAG + Headroom hooks, installer service restart flow, UI consistency), then rewrote the strategic docs — ROADMAP.md (1498→498 lines, task list to strategy), created FINAL_GOAL.md (6 pillars including Specialist Research Agents), and updated .bizar/PROJECT.md. Then three parallel high-priority implementation streams closed 7 gaps each: Stream A (Tyr — background agents dashboard integration: pause/resume, WS streaming, tool call history, active session persistence, progress reporting), Stream B (Thor — memory system integration: 4 plugin tools, session-start injection, session-end write, auto-reindex), Stream C (Thor — installer end-to-end: LightRAG install, env population, headroom lifecycle, post-install smoke test, Alpine/NixOS/Void support, idempotency hardening, API key bootstrap). Final test results: 395/395 backend node:test, 319/324 web vitest (5 pre-existing a11y fails), 633/636 plugin bun (3 pre-existing), 0 TypeScript errors.

**Lessons learned:**

- **Background agents without live streaming are invisible to the operator.** Adding WS message types for `session.progress`, `session.tool_call_start`, and `session.tool_call_end` was the difference between "agent dispatched" and "agent actually doing work." The dashboard must render real-time status, not just poll state files. (plugins/bizar/src/event-stream.ts, bizar-dash/src/server/routes/background.mjs)
- **Session-start memory hooks must be lifecycle-aware.** The `onSessionStart` hook must fire after agent initialization but before the first tool call — too early (before agent init) and the hook has no context; too late and the agent works without memory context. The hook injection point matters as much as the hook content. (plugins/bizar/index.ts)
- **Modular strategic docs beat monolithic task lists.** ROADMAP.md at 1498 lines was unreadable and duplicated issues.md. After the rewrite to 498 lines (strategic orienting doc) with FINAL_GOAL.md as the vision (6 pillars) and issues.md as the task queue, each doc has a distinct purpose. The pattern: ROADMAP = strategy, FINAL_GOAL = vision, issues.md = execution. (ROADMAP.md, FINAL_GOAL.md, .bizar/PROJECT.md)
- **Installer idempotency is a state machine, not a file-copy problem.** The installer must distinguish fresh install vs. upgrade from version X vs. re-run with existing config. Using `install-state.json` (checkpoint file with version + step names) solved this. Each step is idempotent: skip if already completed; redo if version has changed. (install.sh, cli/install-state.mjs)
- **Cross-distro Linux support requires POSIX-level package-manager detection.** Checking `/etc/os-release` is fragile (Void Linux doesn't have standardized fields). Using `which apk` (Alpine), `which nix-env` (NixOS), `which xbps-install` (Void) works across all distros without distro-ID parsing. (install.sh)
- **API key bootstrap must be the last installer step, not the first.** Installing everything first then prompting for keys at the end prevents partial-failure states. If installation fails mid-way, the user does not have dangling credentials. (install.sh)
- **Tool call history is a critical diagnostic primitive.** Background agents stream tool call start/end events with tool name, arguments, and duration — letting the operator see exactly what the agent was doing ("reading memory-store.mjs:34-67") rather than just "status: running." Every long-running agent should expose this. (bizar-dash/src/web/components/BackgroundAgentCard.tsx)

**Pattern to follow next time:**

- For background agent work: always implement live WS streaming (progress, tool calls, status transitions) as part of the dispatch path, not as a follow-up. State-file-only architectures create invisible work streams that the operator cannot trust.
- For multi-stream parallel work: categorize by concern (background agents, memory, installer), not by layer (backend, frontend). Category-disjoint scopes produce zero file collisions and let each stream own its full vertical slice.

**Files changed:** ~30+ files across 3 categories: (A) Background agents — plugin event-stream, background-store, new pause/resume/status WS message types, dashboard BackgroundAgentCard with streaming UI, active session persistence; (B) Memory system — 4 new plugin tools (memory search/read/write/list), session-start injection hook, session-end auto-write hook, LightRAG auto-reindex on writes; (C) Installer — LightRAG install step, full env file population, headroom service lifecycle hooks, post-install smoke test, Alpine/NixOS/Void support, install-state.json idempotency, API key bootstrap prompt at end. Plus: ROADMAP.md (rewrite), FINAL_GOAL.md (new), .bizar/PROJECT.md (updated).

**Agents used:** Tyr (Stream A — background agents dashboard integration), Thor (Stream B — memory system full integration, Stream C — installer end-to-end), Odin (dispatch + synthesis).
...
- **Browser extension MV3 service workers can be terminated by Chrome at any time.** When the service worker disconnects, `chrome.runtime.sendMessage` throws synchronously. The `safeSendMessage()` helper wraps every call in try/catch and falls back to a direct `fetch()` to the dashboard API. Every `sendMessage` in a Manifest V3 extension must be defensive. (`browser-extensions/bizar-clipper/content.ts`)
- **DEP0190 `shell:true` warning should be fixed proactively.** Node.js emits a deprecation warning for `child_process.spawn` with `shell:true` and no arguments. Switching to `shell:false` (all callers already pass array args) silences it and is the correct default. (`bizar-dash/src/server/headroom.mjs`)
