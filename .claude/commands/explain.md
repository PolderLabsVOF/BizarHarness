...
The user is asking a question about the codebase. Use the **Agent tool** to dispatch to a read-only research subagent — name it `frigg` if that agent exists, otherwise use the `general-purpose` or `Explore` subagent type with explicit read-only instructions.
