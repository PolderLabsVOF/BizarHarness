 ...
 *
 *   - `getClineSdkOrThrow` throws a clear error when no serve-info is
 *     available (no cline serve child reachable).
 *   - `spawnBgAgent` returns `cline_serve_unavailable` when the SDK
 *     is unreachable.
 *   - State file persists the new v5.5.1 fields (`liveSession: true`,
 *     `sessionId`, `processId: null`).
 *
 * We don't spawn a real cline serve child here — the SDK is mocked
 * by overriding `BIZAR_SERVE_JSON_PATH` to a missing path so
 * `readServeInfo()` returns null. That makes `getClineSdkOrThrow`
...
const { getClineSdk, getClineSdkOrThrow, subscribeToSession } = await import(
  '../src/server/cline-sdk.mjs'
);

test('getClineSdk returns null when no serve-info is available', async () => {
  const sdk = await getClineSdk();
  assert.equal(sdk, null);
});

test('getClineSdkOrThrow throws with a clear message', async () => {
  await assert.rejects(
    () => getClineSdkOrThrow(),
    /cline_serve_unavailable/,
    'expected getClineSdkOrThrow to throw cline_serve_unavailable',
  );
});
...
test('spawnBgAgent returns cline_serve_unavailable when SDK is missing', async () => {
