...
test('parses single F-NNN header with status + progress + owner', () => {
  ...
  assert.equal(g.title, 'Ship workflow');
...
test('parses the canonical active-feature heading used by root PROGRESS.md', () => {
  const result = parseProgress('## In Progress — F-116 Core harness audit\nGoal is **active**.\n');
  assert.equal(result.goals.length, 1);
  assert.equal(result.goals[0].id, 'F-116');
  assert.equal(result.goals[0].title, 'Core harness audit');
...
test('serialise round-trips goal + KR structure', () => {
  ...
  assert.ok(out.includes('## F-052 — Ship workflow'));
