 ...
 *
 * Sprint S10 — Parse the canonical root `PROGRESS.md` into a structured goal list
 * for the core harness.
 *
 ...
 *
 * Pure core utility with no UI or server dependency.
 */
...
function parseHeader(headerText) {
  const active = /^(?:In Progress|Current|Next|Recent|Backlog)\s*[—-]\s*(F-\d+)\s+(.+)$/i.exec(headerText);
  if (active) return { id: active[1].toUpperCase(), title: active[2].trim() };
...
export function serializeProgress(parsed) {
  ...
  for (const goal of parsed.goals) {
    ...
    // mutating a goal status before serialization would silently no-op
    // on the next read.
