...
- **MCP stdio server:** the agent-browser binary also ships an MCP stdio server. Cline can register it as an MCP tool server so any tool call flows through it natively. See `.cline/mcp.json`.
...
- The agent-browser SKILL.md lives at `~/.cline/skills/agent-browser/SKILL.md` — read it on first use.
- Read, glob, grep
