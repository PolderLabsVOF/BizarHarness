---
name: bizar-docs-lookup
description: Bizar-docs-lookup — Bizar specialist.
isolation: worktree
...
The harness may expose Context7 tools under prefixed names (e.g. an available documentation tool, an available documentation tool). Use the tool names available in your environment (see the agent’s `tools` list).

### Step 1: Resolve the library

Call an available documentation tool for resolving the library ID (e.g. **resolve-library-id** or **an available documentation tool**) with:
...
Call an available documentation tool for querying docs (e.g. **query-docs** or **an available documentation tool**) with:
...
Action: Call the resolve-library-id tool (e.g. an available documentation tool) with libraryName "Next.js", query as above; pick `/vercel/next.js` or versioned ID; call the query-docs tool (e.g. an available documentation tool) with that libraryId and same query; summarize and include middleware example from docs.
