...
function briefing(cwd) {
  try {
    ...
    if (goals.length > 0) return `OpenKan goals: ${goals.length} PRD(s), none active. Use \`bizar goals list\` to choose or \`bizar goals add\` to create one.`;
    return 'OpenKan goals: no PRD yet. Create one with `bizar goals add <title>` when the work needs a durable outcome.';
  } catch {
    return 'OpenKan goals unavailable. Inspect `.ok/` or run `bizar openkan init` before planning.';
  }
}
