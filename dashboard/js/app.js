...
function setupKeyboardShortcuts() {
  const map = { '1': 'overview', '2': 'chat', '3': 'agents', '4': 'plans',
                '5': 'projects', '6': 'config', '7': 'settings' };
...
async function refreshSnapshot() {
  try {
    const [overview, agents, plans, projects, config, settings] = await Promise.all([
      api.get('/overview'),
      api.get('/agents'),
      api.get('/plans'),
      api.get('/projects'),
    ...
    setSnapshot({ overview, agents: agents.agents, plans: plans.plans, projects: projects.projects, config, settings });
  } catch (err) {
