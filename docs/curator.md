...
`config/skills/<name>/SKILL.md` is canonical. `scripts/sync-skills-mirror.mjs` creates the `config/claude/skills` project mirror and reports missing, changed, and orphaned entries. `scripts/verify-thinking-skills.mjs` adds stricter metadata/body checks for the thinking-model library.
