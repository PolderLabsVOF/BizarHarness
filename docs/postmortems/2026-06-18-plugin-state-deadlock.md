...
OpenCode failed to render on startup, then became unable to commit any user message to the conversation database — prompts appeared in the prompt history log but no rows were written to the SQLite store. Six distinct layers of misconfiguration and a re-entrant mutex combined to produce a fully broken state.

---

## Impact

- **Blank startup screen.** OpenCode initialized the Bizar plugin but the plugin never resolved its `init()` promise, blocking the UI render pipeline entirely.
- **Zero message persistence.** After startup was manually progressed, every `chat.message` submission deadlocked on a per-session mutex inside `state.ts`. Prompts were appended to `~/.local/state/opencode/prompt-history.jsonl` but never committed to `~/.local/share/opencode/opencode.db`.
...
| Time | Event |
|------|-------|
| Day N | `~/.config/opencode/plugins/bizar/index.ts` added `client.session.list()` during stale-session cleanup — no timeout wrapper. |
| Day N | `~/.config/opencode/opencode.json` and `~/.opencode/opencode.json` updated to model `minimax/MiniMax-M3`, small_model `minimax/MiniMax-M2.7`, default_agent `quick`, with `supabase.enabled = false`, `hindsight.enabled = false`, MCP permissions switched from `ask` to `deny`. |
| Day N | `~/.config/environment.d/90-hindsight.conf` created for environment-variable persistence. Loaders added to `~/.bashrc`, `~/.profile`, and `~/.config/fish/conf.d/90-hindsight.fish`. `opencode.json` updated with a direct bearer token fallback. |
| Day N | `~/Projects/BizarHarness/config/opencode.json` (project-local override) was still forcing `default_agent: odin`, `model: opencode/deepseek-v4-flash-free`, and re-enabling the broken MCPs. |
| Day N | Agent definition files for `quick`, `frigg`, `vor`, `mimir`, `heimdall` still had `model: opencode/deepseek-v4-flash-free` pinned inside each YAML frontmatter. |
...
| 2026-06-18 | User observes blank screen on OpenCode launch. |
| 2026-06-18 | User bypasses startup block manually, submits a message — UI freezes, no response. |
| 2026-06-18 | User inspects `prompt-history.jsonl` (prompts present) vs `opencode.db` (no rows) — concludes message storage is failing. |
| 2026-06-18 | Six-layer fix applied (see Resolution). OpenCode recovers. |
...
- **`~/.local/state/opencode/prompt-history.jsonl`** — contained entries for submitted prompts, confirming the hook fired and the LLM was invoked.
- **`~/.local/share/opencode/opencode.db`** — zero rows in any message table, confirming `stateStore.save()` was never reached.
...
| File | Change |
|------|--------|
| `~/.config/opencode/plugins/bizar/index.ts` | Wrapped `client.session.list()` in `Promise.race(..., 1000)` with a 1s fallback returning `[]` |
...
| `~/.config/opencode/opencode.json` | Added `hindsight.bearerToken` fallback field |

### Layer 3 — OpenCode Global Config Normalization

| File | Change |
|------|--------|
| `~/.opencode/opencode.json` | Reset to model `minimax/MiniMax-M3`, small_model `minimax/MiniMax-M2.7`, default_agent `quick`, `supabase.enabled: false`, `hindsight.enabled: false`, MCP permissions `deny` |
| `~/.config/opencode/opencode.json` | Same reset as above |

### Layer 4 — Project-Local Config Override

| File | Change |
|------|--------|
| `~/Projects/BizarHarness/config/opencode.json` | Removed `default_agent: odin`, removed `model: opencode/deepseek-v4-flash-free`, re-aligned MCP enable flags to match global baseline |

### Layer 5 — Interactive Agent Model Rerouting

| File | Change |
|------|--------|
| `config/agents/quick.md` | Changed YAML `model` from `opencode/deepseek-v4-flash-free` to `minimax/MiniMax-M2.7` |
...
`~/Projects/BizarHarness/config/opencode.json` was overriding `default_agent`, `model`, and MCP flags in ways that conflicted with the global config. Project-level overrides are powerful but become a second source of truth that can silently re-enable broken features after a global cleanup.
...
The five agent files had `model: opencode/deepseek-v4-flash-free` hard-coded in YAML frontmatter. This bypasses Odin's tier-routing entirely and forces a free-tier model regardless of the task complexity the routing table was designed to dispatch.
...
### OpenCode Runtime / User Config

| Path | Purpose |
|------|---------|
| `~/.opencode/opencode.json` | User-level OpenCode config (global baseline) |
| `~/.config/opencode/opencode.json` | XDG-compliant OpenCode config (global baseline) |
| `~/.config/opencode/plugins/bizar/index.ts` | Bizar plugin entry point; had blocking `client.session.list()` call |
...
| Path | Purpose |
|------|---------|
| `~/Projects/BizarHarness/config/opencode.json` | Project-local OpenCode override; was re-enabling broken models and MCPs |
...
| Path | Purpose |
|------|---------|
| `~/.local/state/opencode/prompt-history.jsonl` | Prompt history log; confirmed prompts were submitted |
| `~/.local/share/opencode/opencode.db` | OpenCode SQLite store; confirmed zero message rows committed |
