...
function recordSuccess(learner, modelId, tier, role = "implementer", status = "success") {
  learner.record({
    ...
    // languageTag is intentionally omitted to match the selector
    // (the canonical bucket key writes all omitted fields as `null`).
    taskKey: {
      modelId,
      tier,
      role,
  ...
  it("sequential record() updates the next call's ranking", () => {
    ...
    for (let i = 0; i < 3; i++) {
      learner.record({
        routingDecisionId: newRoutingDecisionId(),
        modelId: "anthropic/claude-3-5-sonnet",
        taskKey: { modelId: "anthropic/claude-3-5-sonnet", tier: "high", role: "implementer", phase: "implement", capability: "tool-use", riskLevel: "medium" },
        status: "failure",
        verifiedBy: "auto-verifier",
        capturedAt: new Date().toISOString(),
      });
