...
> Module-specific hard rules. These complement the root AGENTS.md
> constraints. Violations break the plugin's portability.
...
- **MUST NOT** return `{ output: JSON.stringify(...) }` from tool
  `execute()`.
...
1. Read `docs/architecture.md` for the layer model.
2. Check `.harness/arch-rules.json` for enforced rules.
